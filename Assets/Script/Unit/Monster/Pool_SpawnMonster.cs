...
public class Pool_SpawnMonster : MonoBehaviour
{
    ...
    public void GetPool(Vector3 position, Vector3 rotation = default(Vector3), Vector3 scale = default(Vector3), float attack_damage = -1, float criticalRate = -1, float criticalDamage = -1)
    {
        ...
        if(transform.GetChild(_refIndex).gameObject.activeSelf == false)
        {
            ...
            if(_isAutoBack == true)
            {
                childInPool.GetComponent<spawnMonsterInHere>().SetPool(this.gameObject);
                childInPool.GetComponent<spawnMonsterInHere>().timeToBack();
            }
            if(attack_damage != -1)
            {
